...
When("I pressed at math operator {string}") do |op_id|
  @base.click_id(ID["app"][op_id])
  @check_result = @app.check_operation(@fst_number, @snd_number, op_id)
end

Then("the result has to be according to the math operation {string}") do |operation|
